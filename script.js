...
function addWeatherDataOnUI(location) {
  ...
  document.getElementById("windSpeed").innerHTML += ' '+
    location.wind.speed + ` kmph`;
  document.getElementById("humidity").innerHTML += ' '+ location.main.humidity;
  document.getElementById("timeZone").innerHTML += ' '+ getTimeZone(
    location.timezone
  );
  document.getElementById("pressure").innerHTML += ' '+ (location.main.pressure*100) * 1.01325e-5 + ` atm`;
  ...
  document.getElementById("feelsLike").innerHTML += ' '+ `${Math.round(
    location.main.feels_like - 273.15
  )}&deg C`;
}
